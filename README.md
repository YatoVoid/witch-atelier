...
- `js/data/signatures.js`: recognizes a drawn spell as a named spellbook entry, but only for the three whose actual sign composition is confirmed: Grasping Wind and Sylph Shoes Seal from the wiki, Light Beam read directly off its reference art (a light sigil with four Column signs at north, east, south, and west). The other 37 entries were never confirmed compositionally, so there's nothing honest to match against; faking that would be worse than not detecting them.
